...
class OverviewRepositoryImpl(
    ...
    override fun getActivities(): Flow<List<DailyActivitiesEntity>> = dailyActivitiesDao.getDailyActivities()

    override suspend fun deleteActivity(activity: Activity) {
        dailyActivitiesDao.deleteActivity(activity.name)
    }

    override suspend fun resetMeal(meal: String) {
        dailyNutritionDao.resetMeal(meal)
    }
}